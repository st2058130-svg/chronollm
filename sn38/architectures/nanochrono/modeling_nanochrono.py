...
class NanochronoAttention(nn.Module):
...
class NanochronoDecoderLayer(nn.Module):
    def __init__(self, config, layer_idx):
        super().__init__()
        self.self_attn = NanochronoAttention(config, layer_idx)
        self.mlp = NanochronoMLP(config)
...
class NanochronoPreTrainedModel(PreTrainedModel):
    config_class = NanochronoConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = False
    _no_split_modules = ["NanochronoDecoderLayer"]
...
class NanochronoModel(NanochronoPreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [NanochronoDecoderLayer(config, i) for i in range(config.num_hidden_layers)]
        )
...
class NanochronoForCausalLM(NanochronoPreTrainedModel, GenerationMixin):
    def __init__(self, config):
        super().__init__(config)
        self.model = NanochronoModel(config)
